...
#===============================================================================
#------------------------------------------------------------------------------
# Monitoring (Log Analytics, Application Insights, Alerts, Dashboard)
#------------------------------------------------------------------------------
module "monitoring" {
...
module "dr" {
  ...
  storage_containers = {
    ...
    processed_container = var.storage.processed_container
  }
  dr = var.dr
...
#===============================================================================
# INTEGRATIONS (Event Grid, Metric Alerts — avoids circular dependencies)
#===============================================================================
module "integrations" {
...
check "prod_dr_must_be_enabled" {
  assert {
    condition     = var.dr.enabled
    error_message = "DR must be enabled in the production environment."
  }
}

check "prod_cmk_must_be_enabled" {
  assert {
    condition     = var.security.enable_customer_managed_key
    error_message = "Customer-managed keys must be enabled in the production environment."
  }
}
