#------------------------------------------------------------------------------
# DR Web Application - Terraform Variables (Grouped Object Pattern)
#------------------------------------------------------------------------------
...
variable "network" {
  ...
  type = object({
    ...
    private_subnet_cidrs  = optional(list(string), ["10.0.10.0/24", "10.0.11.0/24"])
    database_subnet_cidrs = optional(list(string), ["10.0.20.0/24", "10.0.21.0/24"])
  })
}
...
variable "compute" {
  ...
  type = object({
    ...
    asg_desired_capacity       = optional(number, 3)
